...
def fig_R(res):
    ...
    for ax, tp in zip(axes, ("1-2주", "3개월", "12개월")):
        ...
        ax.set_xticks(h)
        ax.set_xlabel("그날의 몇 번째 명목 공부시간", color=INK2, fontsize=9)
        ax.set_ylim(30, 120)
...
def fig_year(res):
    ...
    axes[1].set_title("공부일 하루 총량(RHE: 회복된 첫 시간 몇 개분)", color=INK, fontsize=11)
    for ax in axes:
...
def fig_opt(res):
    ...
    ax.set_ylabel("연간 총량(같은 가정 조합에서 휴일 14일에 1일·최대 시간 = 100)", color=INK2, fontsize=9)
